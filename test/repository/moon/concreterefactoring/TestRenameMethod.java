...
public class TestRenameMethod extends RefactoringTemplateAbstractTest {
	 ...
	 * 
	 * <p>En un modelo con una única clase, toma un método sin argumentos formales,
	 * valor de retorno, ni variables locales, y le asigna un nuevo nombre
	 * correcto, distinto al de los demás métodos de la clase.</p>
	 * 
	 ...
	 * 
	 * <p>En un modelo con dos clases (una extiende a la otra), renombra un
	 * método en la superclase que está siendo redefinido en la subclase,
	 * comprobando que los cambios se extiendan correctamente por la jerarquía
	 * de herencia.</p>
	 * 
	 * @throws Exception si se produce un error durante la ejecución de la prueba.
	 */  
	...
	public void testWithContents() throws Exception{
		...
		List <MethDec> lMetodo3 = classDef.getMethDecByName(factory.createName("nuevoMetodo")); //$NON-NLS-1$
		assertFalse("Test renombrar método con contenidos: no existe el método" + //$NON-NLS-1$
			" con el nuevo nombre.", lMetodo3.isEmpty()); //$NON-NLS-1$
		...
		assertEquals("Test renombrar método con contenidos: no se ha " + //$NON-NLS-1$
			"modificado correctamente el nombre del argumento formal en el método", //$NON-NLS-1$
			factory.createName("paqueteA.ClaseA~nuevoMetodo%String#b"), //$NON-NLS-1$
			argumento.getUniqueName());

		List <LocalDec> ld = metodo3.getLocalDecs();
		LocalDec local = ld.get(0);

		assertEquals("Test renombrar método con contenidos: no se " + //$NON-NLS-1$
			"encuentra la variable local del método.", //$NON-NLS-1$
			factory.createName("paqueteA.ClaseA~nuevoMetodo%String#a"), //$NON-NLS-1$
			local.getUniqueName());
	}
	 ...
	 *
	 * <p>Comprueba que se lanza una excepción cuando se intenta renombrar un 
	 * método y su signatura vaya a coincidir con la de algún otro método de
	 * las clases de la jerarquía de herencia.</p>
	 *
	 * <p>En un modelo con dos clases (la una hereda de la otra), toma un método de
	 * la subclase y lo renombra de manera que su signatura vaya a coincidir con
	 * la de un método de la superclase.</p>
	 * 
	 * @throws Exception si se produce un error durante la ejecución de la prueba.
	 */
	 ...
	 *
	 * <p>Comprueba que se lanza una excepción cuando se intenta renombrar un 
	 * método constructor de su clase.</p>
	 * 
	 * @throws Exception si se produce un error durante la ejecución de la prueba.
	 */
	@Test(expected=PreconditionException.class)
	...
	public void testUndoSimple() throws Exception{
		...
		assertTrue("Test deshacer renombrado de método simple: t" + //$NON-NLS-1$
			"todavía existe el método con el nuevo nombre", lMetodo3.isEmpty()); //$NON-NLS-1$
	}

	/** 
	 * Comprueba que la refactorización se deshace correctamente al 
	 * deshacer un renombrado de un método en una superclase.
	 * 
	 * @throws Exception si se produce un error durante la ejecución de la prueba.
	 */  
	@Test
	public void testUndoWithInheritance() throws Exception{
		...
		assertFalse("Deshacer renombrado de método con herencia: " + //$NON-NLS-1$
			"no existe el método con el nombre original.", lMetodo2.isEmpty()); //$NON-NLS-1$
		...
		List <MethDec> lMetodo4 = classDef2.getMethDecByName(factory.createName("metodo1")); //$NON-NLS-1$
		assertFalse("Deshacer renombrado de método con herencia: " + //$NON-NLS-1$
			"no existe el método con el nombre original.", lMetodo4.isEmpty()); //$NON-NLS-1$

		List <MethDec> lMetodo5 = classDef2.getMethDecByName(factory.createName("renombrado")); //$NON-NLS-1$
		assertTrue("Deshacer renombrado de método con herencia: " + //$NON-NLS-1$
			"sigue existiendo el método renombrado por herencia.", lMetodo5.isEmpty()); //$NON-NLS-1$
	}
