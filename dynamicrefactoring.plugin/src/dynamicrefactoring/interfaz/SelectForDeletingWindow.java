 ...
 * borrado de la refactorizaci�n.</p>
 *
...
public class SelectForDeletingWindow extends SelectDynamicRefactoringWindow {
		...
		if(buttonId == IDialogConstants.OK_ID) {
			...
			if (l_Available.getSelectionCount() == 1){
				...
				if (isConfirmed(refactoring.getName())){
					...
					// Si el fichero de la refactorizaci�n no se encuentra en un
					// subdirectorio, se borra solamente dicho fichero.
					if (dirPath.length() == 0)
						deleted = FileManager.deleteFile(filePath);
					// Si se encuentra en un directorio, se borra �ste y todo su 
					// contenido.
					else {
	 ...
	 * din�mica.
	 * 
	 * @param name nombre de la refactorizaci�n seleccionada para ser eliminada.
	 * 
	 * @return <code>true</code> si el usuario da su confirmaci�n para eliminar la
	 * refactorizaci�n seleccionada; <code>false</code> en caso contrario. 
	 */
