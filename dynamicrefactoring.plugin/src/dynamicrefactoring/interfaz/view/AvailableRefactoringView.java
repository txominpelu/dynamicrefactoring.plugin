...
public class AvailableRefactoringView extends ViewPart {
	...
	private static final Logger logger = 
		Logger.getLogger(AvailableRefactoringView.class);
	...
	/**
	 * Última seleción válida como entrada para una refactorizacion en el espacio de trabajo.
	 */
	...
	public void createPartControl(Composite parent) {
			...
			SelectionListenerRegistry.getInstance().addListener(new MainSelectionListener());
	...
	/**
	 * Rellena el árbol con las refactorizaciones disponibles para el elemento seleccionado.
	...
	public void fillTree(HashMap<String, String> refactorings){
		...
		for(Map.Entry<String, String> nextRef : refactorings.entrySet()){
			TreeItem refactoring = TreeEditor.createBranch(tr_Refactorings, i,
					nextRef.getKey(), "icons" + System.getProperty("file.separator") + "ref.png"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
			i++;
			try{
				if(nextRef.getValue() != ""){ //$NON-NLS-1$
					...
					preconditionsChild.setImage(ResourceManager.getPluginImage(
							RefactoringPlugin.getDefault(), "icons"  //$NON-NLS-1$
					...
					TreeEditor.fillInTreeBranch(definition.getPostconditions(), postconditionsChild, 
							"icons" + System.getProperty("file.separator") + "validate.gif");		 //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
				}
			}
			catch(RefactoringException e){
				logger.error(
					Messages.AvailableRefactoringView_NotRepresented
	 ...
	 * válido como entrada para una refactorización{@link AvailableRefactoringView}.
	 * 
	 * @author <A HREF="mailto:lfd0002@alu.ubu.es">Laura Fuente de la Fuente</A>
	 */
		...
		public void elementSelected(SelectionInfo selection){
			if (selection.isValidSelectionType()){
				...
				try{
					...
					logger.error(
							Messages.AvailableRefactoringView_ReaderFail
							+ ".\n" + e.getMessage()); //$NON-NLS-1$
				}
			}else{
				cleanView();
			...
			if(selection[0].getParentItem() == null){
				try{
					if (saveUnsavedChanges()){
						...
						RefactoringPlugin mediator = 
							RefactoringPlugin.getDefault();
						
						SelectionHandlerFactory factory = 
							SelectionHandlerFactory.getInstance();
						ISelectionHandler handler =
							factory.createHandler(select);
						
						if (handler != null)
							mediator.runRefactoring(
								handler.getMainObject(), select,false);
						
							String name = FileManager.getFileName(
								refactorings.get(selectedName));
							new DynamicRefactoringWindowLauncher(
								handler.getMainObject(), FileManager.getFilePathWithoutExtension(name));
					}
	 ...
	 * 
	 * @return <code>true</code> si se guardaron las modificaciones pendientes o si
	 * no había ninguna pendiente; <code>false</code> en caso contrario. 
	 */
	public boolean saveUnsavedChanges() {
		...
		if (dirtyEditors.length > 0){
			...
			if (MessageDialog.openConfirm(
					window.getShell(),"WARNING", //$NON-NLS-1$
