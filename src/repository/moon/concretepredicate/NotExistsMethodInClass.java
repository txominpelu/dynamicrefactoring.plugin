/*<Tarea Ant RefactoringPlan - Plugin that allows to perform refactorings 
...
public class NotExistsMethodInClass extends Predicate {
	...
	public NotExistsMethodInClass(MethDec method, ClassDef clase) {
		...
		super("NotExistsMethodInClass: \n\t" + //$NON-NLS-1$
			  "Makes sure the given method " + '"' + method.getUniqueName() + '"' +  //$NON-NLS-1$
			  " does not already belong to " + '"' + method.getClassDef().getName() +  //$NON-NLS-1$
			  '"' + ".\n\n"); //$NON-NLS-1$
		
		this.oppositePredicate = 
			new ExistsMethodInClass(method,clase);
	}
